...
it_succeeds_with_PygmentsAndPackager() {
   ...
   queryPackager="$(rpm -qp "${tmpDir}/Pygments-1.5-1.noarch.rpm" --qf '%{PACKAGER}')"
   if [ "${queryPackager}" != "a sonbitch packager" ]
   then
      echo "queried package name ${queryPackager} does not match expected name:  a sonbitch packager" 1>&2
      return 1
   fi
